...
class Registry {
	...
	private $noVersionsToKeep = 100;

	function __construct($registryUrl) {
	...
	public function setNoVersionsToKeep($noVersionsToKeep) {
...
if ($argc === 1) {
	echo $argv[0] . " registry-url number-of-versions-to-keep [delete]" . PHP_EOL;
	exit;
} 

$registry = new Registry($argv[1]);

if (count($argv) > 2) {
	$registry->setNoVersionsToKeep($argv[2]);
}

if (count($argv) > 3 && $argv[3] === 'delete') {
